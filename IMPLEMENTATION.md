...
2. **Browser retrieval (implemented, unverified live):** `scripts/retrieve-uarb.mjs` attempts matter search, five tab counts, pagination when fewer than ten rows are visible, and up to ten “Go Get It” downloads. It emits a JSON result. Live DOM, count semantics, metadata labels, pagination controls, and download behavior still need verification when the site responds.
