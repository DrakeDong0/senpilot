...
4. **Email workflow (partially implemented):** `senpilot/mail.py` can fetch unseen plain-text requests over IMAP without marking them read and send a prepared reply over SMTP/TLS. `senpilot/reply.py` composes MIME replies, including clarification for ambiguous requests. The adapters are not yet wired to a browser retrieval worker, and no live mailbox has been tested.
